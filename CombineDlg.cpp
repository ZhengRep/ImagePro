...
BOOL CCombineDlg::OnInitDialog() 
{
	...
	while (posTemplate != NULL) 
	{
		...
		while (posDoc != NULL) 
		{
			CImageProDoc* pDoc = (CImageProDoc*) pTemplate->GetNextDoc(posDoc);
			ASSERT(pDoc->IsKindOf(RUNTIME_CLASS(CImageProDoc)));
...
void CCombineDlg::OnOK() 
{
	...
	CImageProDoc* pRedDoc = (CImageProDoc*) m_ctrlRedChannel.GetItemData(m_ctrlRedChannel.GetCurSel());
	...
	CImageProDoc* pGreenDoc = (CImageProDoc*) m_ctrlGreenChannel.GetItemData(m_ctrlGreenChannel.GetCurSel());
	...
	CImageProDoc* pBlueDoc = (CImageProDoc*) m_ctrlBlueChannel.GetItemData(m_ctrlBlueChannel.GetCurSel());
