...
public:
	...
	BOOL ReplaceColorPal(CDibObject *pDibObject , BYTE *bpColorsTable);//byte 数据类型
	...
	int *CreateHueHist(int Thresh, CDibObject *pDibObject = NULL );//创建色相色调直方图
	//thresh 阈值
	...
	int FindThresh_Discriminant( CDibObject *pDibObject = NULL );//discriminant 判别式
	...
	int *GetHistogram( int nX1 = -1, int nY1 = -1, int nX2 = -1, int nY2 = -1, CDibObject *pDibObject = NULL );//获得直方图
