...
////////////////////////////////////////////////////////////////////////
//BOOL MakeGray256(BYTE mGrayType, CDibObject *pDibObject)   彩色图像转换为灰度图像 mGrayType绝对转换为灰度图的方式，R,G,B
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
////////////////////////////////////////////////////////////////////////
BOOL CPointPro::MakeGray256(BYTE mGrayType,CDibObject *pDibObject )//转换为灰度图
{
	//CDibObject对象指针
	...
	//定义变量
	BITMAPINFOHEADER *pOldBIH, *pNewBIH;//bitmap info header
	BITMAPFILEHEADER *pOldBFH, *pNewBFH;//bitmap file header
	...
	DWORD dwSize;
	//用palette索引来存储图像
	dwSize = sizeof( BITMAPFILEHEADER ) + sizeof( BITMAPINFOHEADER ) + 256 * sizeof( RGBQUAD ) + m_pDibObject->GetHeight() * nNewWidthBytes;
		...
		case 0:	//按亮度Y=0.3R+0.59G+0.11B将彩色图像转换为灰度图像
			for( i=0; i<256; i++ )
			{
				dwGray = ((DWORD)pOldRGBPalette[i].rgbRed * 30 +
					...
					(DWORD)pOldRGBPalette[i].rgbBlue * 11) / 100;//避免最后为小数之和
...
/////////////////////////////////////////////////// /////////////////////
//int FindThresh( CDibObject *pDibObject )    threshhold value 阈值
//----------------------------------------------------------------------
									 ...
									 CDibObject *pDibObject)
{
	...
	//创建灰度映射表 创建映射表，通过查表有效降低每个像素计算的开销，可能存在很多重复的计算
